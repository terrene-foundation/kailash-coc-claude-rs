...
- Key settings to prevent blocking: Use `NexusApp()` from `kailash.nexus` and register workflows manually
...
4. > Use the framework-advisor subagent to recommend Core SDK vs kailash.DataFlow vs Nexus vs Kaizen for [feature]
   - If kailash.DataFlow recommended: > Use the dataflow-specialist subagent for implementation details
...
2. > Use the pattern-expert subagent to implement [component] following SDK patterns
   - For kailash.DataFlow components: > Use the dataflow-specialist subagent for database patterns
...
3. > Use the testing-specialist subagent to understand test failures
   - For kailash.DataFlow issues: > Use the dataflow-specialist subagent for database-specific debugging
...
- **`.claude/skills/01-core-sdk/SKILL.md`**: Essential SDK patterns navigation
...
- **`.claude/skills/15-error-troubleshooting/`**: Error solutions
...
```
1. > Use the framework-advisor subagent to confirm kailash.DataFlow is appropriate
...
```
For kailash.DataFlow + Nexus integration:
