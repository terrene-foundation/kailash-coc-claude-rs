...
```bash
# Check for Kailash (Rust-backed Python package)
grep -rl "kailash-enterprise\|kailash" requirements.txt pyproject.toml setup.py setup.cfg 2>/dev/null
grep -rl "from kailash\|import kailash" --include="*.py" src/ app/ lib/ 2>/dev/null
```

**Report your detection result before proceeding:**

- "Kailash SDK detected" → Apply universal + Kailash checks
...
- [ ] All imports use `import kailash` (direct API)
- [ ] No relative imports, no submodule imports

### Runtime Execution Pattern

- [ ] Always: `result = rt.execute(builder.build(reg))`
- [ ] Never: `workflow.execute(runtime)` or `rt.execute(workflow)` (missing `.build(reg)`)
...
- PASS/FAIL: kailash.DataFlow (N violations)
