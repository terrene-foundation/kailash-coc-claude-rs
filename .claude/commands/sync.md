...
This repo inherits its `.claude/` directory from a USE template (`kailash-coc-claude-rs`). The template is updated when the kailash/ source runs `/sync`. This command pulls those updates into your repo.

```
kailash/ (source) → kailash-coc-claude-rs/ (USE template) → THIS REPO
                                                              ↑ you are here
```
...
2. Open kailash/ and run `/sync rs`
