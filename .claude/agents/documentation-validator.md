...
- `.claude/skills/17-gold-standards/` — Documentation and compliance standards
- `.claude/skills/07-development-guides/` — Development and implementation guides
...
| Missing .build(reg) | `rt.execute(workflow)` | Add `.build(reg)` call |
| Wrong param count | 2-param connections | Use 4-param pattern |
| Deprecated pattern | `workflow.execute()` | Use `rt.execute()` |
