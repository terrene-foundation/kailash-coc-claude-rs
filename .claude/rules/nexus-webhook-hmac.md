...
Origin: gh-coc-claude-rs#51 items 1A + 1C (2026-04-17). kailash-rs Nexus handlers receive pre-parsed JSON; `axum::extract::Json` consumes the raw body before the handler runs, and the handler signature accepts only `Query + Json` (no `HeaderMap`, no raw body access). The runtime fix that exposes raw bytes to handlers is a separate design track (decisions D1–D6 pending human approval). Until the runtime fix lands, every HMAC webhook consumer MUST externalize verification per this rule; once raw-body exposure lands, this rule will be revised to document the DO pattern using the new handler signature.
